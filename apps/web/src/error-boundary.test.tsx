...
describe('SentryErrorBoundary', () => {
  ...
  it('should call Sentry.withScope when an error is caught', () => {
    ...
    render(
      <SentryErrorBoundary fallback={<div>Error occurred</div>} context="test-context">
        <ThrowingComponent />
      </SentryErrorBoundary>,
    )
  ...
  it('should set errorBoundary tag with context prop', () => {
    ...
    render(
      <SentryErrorBoundary fallback={<div>Error occurred</div>} context="booking-page">
        <ThrowingComponent />
      </SentryErrorBoundary>,
    )
    expect(mockSetTag).toHaveBeenCalledWith('errorBoundary', 'booking-page')
    vi.restoreAllMocks()
  })
})
