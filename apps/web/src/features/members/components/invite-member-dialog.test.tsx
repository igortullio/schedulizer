...
describe('InviteMemberDialog', () => {
  it('renders email input and role select when open', () => {
    render(<InviteMemberDialog isOpen={true} error={null} onClose={vi.fn()} onSubmit={vi.fn()} />)
  ...
  it('calls onSubmit with email and default role on form submit', async () => {
    ...
    render(<InviteMemberDialog isOpen={true} error={null} onClose={vi.fn()} onSubmit={onSubmit} />)
    await user.type(screen.getByTestId('invite-email-input'), 'test@test.com')
  ...
  it('calls onClose when cancel button is clicked', async () => {
    const user = userEvent.setup()
    const onClose = vi.fn()
    render(<InviteMemberDialog isOpen={true} error={null} onClose={onClose} onSubmit={vi.fn()} />)
